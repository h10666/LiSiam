...
'''define all'''
__all__ = ['CrossEntropyLoss', 'BinaryCrossEntropyLoss', 'invarianceLoss']
...
def CrossEntropyLoss(prediction, target, scale_factor=1.0, **kwargs):
    ...
    # print('##target:',target.size(),target[:5].cpu().data)
    # print('##prediction:',prediction.size(),prediction[:5].cpu().data)
...
def invarianceLoss(prediction, target, scale_factor=1.0, **kwargs):
    # calculate the loss
    # loss_er = torch.mean(torch.abs(prediction[:,1:,:,:]-target[:,1:,:,:]))
    loss_iv = torch.mean(torch.abs(prediction-target))
    loss_iv = loss_iv * scale_factor
    # print('## each loss_iv:',loss_iv)
    return loss_iv
