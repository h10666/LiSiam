...
class Xception(nn.Module):
    ...
    def features(self, x):
        ...
        outs = []
        for i, feats in enumerate([x2, x3, x4]):
...
'''build resnet'''
def BuildXceptionBig(xception_type, **kwargs):
    # parse args
    default_args = {
        'use_stem': True,
        'norm_cfg': None,
    ...
    if os.path.exists(default_args['pretrained_model_path']):       # default_args['pretrained'] and 
        print('##load pretrained_model from ', default_args['pretrained_model_path'])
        ...
        print('##load pretrained_model from ', default_args['pretrained'])
