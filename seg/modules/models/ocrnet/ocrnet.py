...
# LiSiam_AS5
class LiSiam_A(OCRNetSABase):
    def __init__(self, cfg, **kwargs):
        super(LiSiam_A, self).__init__(cfg, **kwargs)    
    ...
    def forward(self, x, targets=None, images2=None, targets2=None, losses_cfg=None):
        ...
        predsLabel = self.clsDecoder(feats_cls, mask_foreg, mask_backg) # feats_end, 
        ...
        return preds, predsLabel   #,feats_end
...
# LiSiam
# cls1+cls2+seg1+seg2+li+MT
class LiSiam_B(OCRNetSABase):
    def __init__(self, cfg, **kwargs):
        super(LiSiam_B, self).__init__(cfg, **kwargs)    
        decoder_cfg = cfg['decoder']
...
## cls1+li+MT
# LiSiam_AS7
class LiSiam_C(OCRNetSABase):
    def __init__(self, cfg, **kwargs):
        super(LiSiam_C, self).__init__(cfg, **kwargs)    
        decoder_cfg = cfg['decoder']
...
# LiSiam_AS3
# seg2+cls2+li+MT
class LiSiam_D(OCRNetSABase):
    def __init__(self, cfg, **kwargs):
        super(LiSiam_D, self).__init__(cfg, **kwargs)    
        decoder_cfg = cfg['decoder']
...
# LiSiam_AS2
# seg2+cls2+MT
class LiSiam_E(OCRNetSABase):
    def __init__(self, cfg, **kwargs):
        super(LiSiam_E, self).__init__(cfg, **kwargs)    
        decoder_cfg = cfg['decoder']
...
# LiSiam_AS4
# seg1+cls1+li
class LiSiam_F(OCRNetSABase):
    def __init__(self, cfg, **kwargs):
        super(LiSiam_F, self).__init__(cfg, **kwargs)    
        decoder_cfg = cfg['decoder']
...
## remove second branch and retain inv loss 
# LiSiam_AS5
# the inputs of self-attention: fake feature and entire feature
class TNet_fa(OCRNetSABase):
    def __init__(self, cfg, **kwargs):
        super(TNet_fa, self).__init__(cfg, **kwargs)    
        decoder_cfg = cfg['decoder']
...
#        unFreezeLayers = [self.segDecoder, self.auxiliary_decoder, self.object_context_block, self.spatial_gather_module, self.bottleneck, self.backbone_net]
    ...
    #     feats_end = feats_end.view(feats_end.size(0), -1)
    #     score = self.getScore(feats_end)
    #     return feats_end, score

    def clsDecoder(self, feats_cls, mask_foreg, mask_backg):
        Q_feat =  feats_cls*mask_foreg
        KV_feat = feats_cls*mask_backg
        sa_feats = self.BN(self.SAFusion(Q_feat, feats_cls))
        feats_end = self.se(torch.cat((feats_cls, sa_feats),1))
        feats_end = F.adaptive_avg_pool2d(feats_end, (1, 1))
        feats_end = feats_end.view(feats_end.size(0), -1)
        score = self.getScore(feats_end)
...
# LiSiam_AS5
# the inputs of self-attention: fake feature and entire feature
class TNet_ff(OCRNetSABase):
    def __init__(self, cfg, **kwargs):
        super(TNet_ff, self).__init__(cfg, **kwargs)    
        decoder_cfg = cfg['decoder']
...
#        unFreezeLayers = [self.segDecoder, self.auxiliary_decoder, self.object_context_block, self.spatial_gather_module, self.bottleneck, self.backbone_net]
    ...
    def clsDecoder(self, feats_cls, mask_foreg, mask_backg):
        Q_feat =  feats_cls*mask_foreg
        # KV_feat = feats_cls*mask_backg
        sa_feats = self.BN(self.SAFusion(Q_feat, Q_feat))
        feats_end = self.se(torch.cat((feats_cls, sa_feats),1))
        feats_end = F.adaptive_avg_pool2d(feats_end, (1, 1))
        feats_end = feats_end.view(feats_end.size(0), -1)
        score = self.getScore(feats_end)
