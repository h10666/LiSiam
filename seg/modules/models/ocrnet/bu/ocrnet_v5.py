...
################## add by wj
    ...
    def clsDecoder(self, feats, feats_mid):
        ...
        preds_mask = self.ReLu(feats_mid)
        ...
        Q_feat = torch.mul(feats0, preds_mask)
        # print('##Q_feat:',Q_feat.size())
        
        # Q_feat_1 = self.maskFeatEncoder(Q_feat)
        ...
        sa_feats = self.ReLu(self.saE(self.SAFusion(Q_feat, feats0)))
        # print('##sa_feats1:',sa_feats.size())
        # sa_feats = sa_feats)
        # print('##sa_feats2:',sa_feats.size())
...
##feats2: torch.Size([16, 512, 32, 32])
##preds_mask: torch.Size([16, 512, 32, 32])
##sa_feats1: torch.Size([16, 512, 32, 32])
##sa_feats2: torch.Size([16, 512, 16, 16])
...
####################################
    ...
    def forward(self, x, targets=None, images2=None, targets2=None, losses_cfg=None):
        ...
        # print('##feats:',feats.size())
        feats_mid = self.MF(feats)
        # print('##feats_mid:',feats_mid.size())
        # feed to segDecoder
        preds = self.segDecoder(feats_mid)         ## [BS, 2, 64, 64] (mask)
        # print('##preds:',preds.size())
        
        predsLabel = self.clsDecoder(feats, feats_mid)
...
##feats_mid: torch.Size([16, 512, 32, 32])
...
################ add by wj
        if self.mode == 'TRAIN' and targets!=None:
            ...
            feats_mid_2 = self.MF(feats_2)
            # feed to decoder
            preds_2 = self.segDecoder(feats_mid_2)     ## [BS, 2, 64, 64]
            # print('\n\n##preds_2:',preds_2)
            # print('##preds_2:',preds_2[0,:,30:35,30:35])
            predsLabel_2 = self.clsDecoder(feats_2, feats_mid_2)
