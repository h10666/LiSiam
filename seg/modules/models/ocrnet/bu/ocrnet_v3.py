...
################## add by wj
    ...
    #     pred_mask_2 = F.interpolate(pred_mask_1, size=(16, 16), mode='bilinear', align_corners=self.align_corners)
    ...
    #     Q_feat = torch.mul(feats_2, pred_mask_2)+feats_2
        
    #     feats_2 = self.ReLu(self.SAFusion(Q_feat, feats_2))
...
################ add by wj
        if self.mode == 'TRAIN' and targets!=None:
            ...
            # feed to decoder
            preds_2 = self.decoder1(feats_2)     ## [BS, 2, 64, 64]
            # print('\n\n##preds_2:',preds_2)
